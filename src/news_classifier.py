...
def _priority_score(item: NewsItem) -> int:
    text = f"{item.title} {item.summary}".lower()
    if any(k in text for k in ["나스닥100", "nasdaq 100", "qqq", "qqqm"]):
        return 1
    if any(k in text for k in ["s&p 500", "s&p500", "spy", "voo"]):
        return 2
    if any(k in text for k in ["2641", "일본 지수", "nikkei", "topix", "일본 etf"]):
        return 3
    if any(k in text for k in ["원달러", "usd krw", "엔화", "jpy krw", "usd jpy", "원엔"]):
        return 4
    if any(k in text for k in ["미국 금리", "일본은행", "boj", "한국은행"]):
        return 5
    return 9


def select_top_news(news: Dict[str, List[NewsItem]], limit: int = MAX_NEWS_ITEMS_DEFAULT) -> List[NewsItem]:
    combined = news.get("index", []) + news.get("fx", [])
    ranked = sorted(
        combined,
        key=lambda n: (_priority_score(n), -(n.published_at.timestamp() if n.published_at else 0)),
    )
    return ranked[: max(0, limit)]


def build_brief(news: Dict[str, List[NewsItem]], limit: int = MAX_NEWS_ITEMS_DEFAULT) -> Dict:
    top_news = select_top_news(news, limit=limit)
    ...
    points = [n.title for n in top_news[:3]]
    return {
        ...
        "top_news": [
            {
                "title": n.title,
                "source": n.source,
                "published": n.published_text,
                "link": n.link,
                "summary_ko": _simple_summary(n),
                "comment": _invest_comment(n),
            }
            for n in top_news
        ],
        "interpretation": [
